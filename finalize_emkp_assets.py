...
def format_hours_minutes_seconds(total_seconds):
...
def get_mp4_duration(path):
    ...
    if normalize_final_extension(path) != ".mp4":
        return ""
    ...
    try:
        ...
        return format_hours_minutes_seconds(
            frame_count / fps
        )
...
def main():
    ...
    for (
        ...
        if len(matches) == 0:
            ...
            if folder_name in DELETE_FOLDER_NAMES:

                skipped_deleted_folders.append(
                    (
                        folder_name,
                        item_id,
                    )
    ...
    if skipped_deleted_folders:

        print()

        print(
            "Already deleted / absent folders:"
        )

        for (
            folder_name,
            item_id,
        ) in skipped_deleted_folders:

            print(
                f"  {folder_name} -> {item_id}"
            )
    ...
    # IMPORTANT
    #
    # Sequence numbering is based on the current sorted
    # source-file list. Deleted files leave intentional
    # numbering gaps.
    # --------------------------------------------------------
    ...
    if DRY_RUN:
        ...
        print(
            f"  Original assets:    {total_assets:,}"
        )


        print(
            f"  Delete assets:      {deleted_assets:,}"
        )


        print(
            f"  Retained assets:    {retained_assets:,}"
        )
